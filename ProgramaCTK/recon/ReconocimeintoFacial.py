...
def Reconocer():
	...
	#face_recognizer = cv2.face.FisherFaceReco|gnizer_create()
	face_recognizer = cv2.face.LBPHFaceRecognizer_create()

	# Leyendo el modelo
	#face_recognizer.read('modeloEigenFace.xml')
	#face_recognizer.read('modeloFisherFace.xml')
	face_recognizer.read(RutaArchivos + 'modeloLBPHFace.xml')
	...
	while Autorizado == False:
		...
		for (x,y,w,h) in faces:
			...
			if result[1] < 70:
				...
				cv2.putText(frame,'Rostro desconocido',(x,y-20),2,0.8,(0,0,255),1,cv2.LINE_AA)
				cv2.rectangle(frame, (x,y),(x+w,y+h),(0,0,255),2)
